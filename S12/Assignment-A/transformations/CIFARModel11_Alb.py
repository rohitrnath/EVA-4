...
g_alb_pil_transform_train_A11 = Compose([       #For assignment 11
    PadIfNeeded(40,40),             #   Result size of image after padding with 4,4 (4+32+4)
    RandomCrop(32,32),              #   Crop of 32x32
    HorizontalFlip(),               #   FlipLR - Flip left to right
    Cutout(8,8),                    #   Cutout 8X8
    Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
    ToTensor()
])
...
g_alb_train_set = AlbumentationPILImageDataset(image_list = g_train_set, augmentation = g_alb_pil_transform_train_A11)
...
class CIFARModel11_Alb:
    def __init__(self, model,criterion, optimizer = None, scheduler = None, lr = 0.05, momentum = 0.9, step_size = 0, gamma = 0.01):
        self.m_train_losses = []
        self.m_test_losses = []
        self.m_train_acc = []
        self.m_test_acc = []
    ...
    def load_cifar_data(self,train_set,test_set):
        ...
        self.m_train_loader = torch.utils.data.DataLoader(train_set, batch_size=200,
                                          shuffle=True, num_workers=4)
        self.m_test_loader = torch.utils.data.DataLoader(test_set, batch_size=200,
